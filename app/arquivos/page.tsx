          ...
          <div className="max-w-7xl mx-auto">
            ...
            <header className="mb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center justify-between gap-4">
                ...
                {!selectedCustomer && (
                  <div className="bg-white border border-slate-200/80 rounded-2xl flex items-center px-4 h-12 shadow-sm focus-within:border-[#1FA84A] focus-within:ring-4 focus-within:ring-[#1FA84A]/10 transition-all w-full md:w-[300px]">
              ...
              <nav className="inline-flex items-center text-sm font-bold text-slate-500 overflow-x-auto no-scrollbar whitespace-nowrap bg-white border border-slate-200/80 p-1.5 rounded-xl shadow-sm mt-6">
                <button 
                  ...
                  filteredFolders.map(folder => (
                    ...
                    >
                      ...
                      <div className="relative mb-3 mt-2">
                        {/* NOVO ÍCONE DE PASTA SVG SÓLIDO */}
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-24 h-24 text-yellow-400 group-hover:scale-105 transition-transform drop-shadow-sm">
                          <path d="M19.5 21a3 3 0 0 0 3-3v-4.5a3 3 0 0 0-3-3h-15a3 3 0 0 0-3 3V18a3 3 0 0 0 3 3h15ZM1.5 10.146V6a3 3 0 0 1 3-3h5.379a2.25 2.25 0 0 1 1.59.659l2.122 2.121c.14.141.331.22.53.22H19.5a3 3 0 0 1 3 3v1.146A4.483 4.483 0 0 0 19.5 9h-15a4.483 4.483 0 0 0-3 1.146Z" />
                        </svg>
                        
                        {/* BADGE DE CONTADOR CORRIGIDO E SEGURO */}
                        <div className="absolute top-0 right-0 bg-slate-800 text-white text-[11px] font-black w-7 h-7 rounded-full flex items-center justify-center border-2 border-white shadow-sm transform translate-x-1 -translate-y-1 z-10">
                          {folder.tickets.length}
                        </div>
                      </div>
                      
                      <h3 className="font-extrabold text-slate-800 text-[15px] truncate w-full relative z-10" title={folder.contact.name || folder.contact.number}>{folder.contact.name || folder.contact.number}</h3>
              ...
              <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {selectedCustomer.tickets.map(ticket => (
                    ...
                    >
                      {/* CABEÇALHO CORRIGIDO: LIXEIRA SEGURA NO FLEX-BOX */}
                      <div className="flex items-start justify-between gap-3 mb-4 border-b border-slate-100 pb-4">
                        <div className="flex items-center gap-3 flex-wrap">
                          <div className={`w-12 h-12 shrink-0 rounded-xl flex items-center justify-center shadow-inner ${ticket.isArchived ? 'bg-amber-100 text-amber-500' : 'bg-[#e8f6ea] text-[#1FA84A]'} group-hover:scale-110 transition-transform`}>
                          ...
                          {ticket.isArchived && (
                            <span className="bg-amber-100/80 text-amber-700 text-[10px] px-2.5 py-1 rounded-lg font-extrabold uppercase tracking-widest border border-amber-200 shrink-0">Arquivada</span>
                          )}
                        </div>
                        ...
                        {ticket.isArchived && (
                          <button 
                            ...
                            className="w-9 h-9 shrink-0 rounded-xl bg-white border border-red-100 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white hover:border-red-500 transition-all opacity-0 group-hover:opacity-100 shadow-sm"
            ...
            {selectedTicket && (
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                {/* UPLOAD SEGURO (NOME NÃO ESMAGA INPUT) */}
                {pendingFile ? (
                  <div className="bg-white border border-[#1FA84A]/30 shadow-md rounded-2xl p-6 flex flex-col md:flex-row gap-6 items-center mb-6">
                    <div className="flex-1 flex items-center gap-4 min-w-0 w-full">
                      <div className="w-14 h-14 bg-blue-50 text-blue-500 rounded-2xl flex items-center justify-center shadow-inner shrink-0">
                      ...
                      <div className="overflow-hidden min-w-0 flex-1">
                        <h4 className="font-extrabold text-slate-800 text-base truncate w-full" title={pendingFile.name}>{pendingFile.name}</h4>
                  ...
                  <div 
                    ...
                    className="w-full bg-white border-2 border-dashed border-slate-300 rounded-3xl p-8 flex flex-col items-center justify-center cursor-pointer hover:bg-[#1FA84A]/5 hover:border-[#1FA84A]/50 hover:shadow-inner transition-all mb-6 group"
                  >
                    ...
                    <span className="font-extrabold text-slate-700 text-lg group-hover:text-[#1FA84A] transition-colors">Anexar Ficheiro à OS</span>
                  </div>
                )}

                {/* LISTAGEM DE ARQUIVOS COM NOME SEGURO */}
                      ...
                      <div key={file.id} className="bg-white border border-slate-200/80 rounded-2xl flex flex-col hover:shadow-lg hover:-translate-y-1 transition-all group overflow-hidden">
                        <div className="p-5 flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0 pr-8 relative">
                            ...
                            {/* AÇÕES ABSOLUTAS SEGURAS */}
                            <div className="absolute right-0 top-0 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white pl-1 shadow-sm rounded-md border border-slate-100">
                        ...
                        <div className="mt-auto border-t border-slate-100 bg-slate-50/50 px-5 py-3">
                           <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{new Date(file.createdAt).toLocaleDateString('pt-PT')}</span>
                        </div>
                      </div>
                    ))}
                  </div>
