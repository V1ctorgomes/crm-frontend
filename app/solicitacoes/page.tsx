...
export default function SolicitacoesPage() {
  ...
  const handleDeleteNote = (noteId: string) => {
    setConfirmModal({
      isOpen: true,
      title: "Apagar Nota?",
      message: "Tem a certeza que deseja apagar esta nota? Esta ação é irreversível e o registo será perdido.",
      onConfirm: async () => {
        try {
  ...
  const handleToggleArchive = async (ticketId: string, archive: boolean) => {
    try {
      ...
      if (!archive) {
        ...
        showFeedback('success', 'OS enviada para os arquivos.');
      }
            ...
            <div className="w-[320px] bg-slate-50/80 border-r border-slate-200 flex flex-col shrink-0 hidden md:flex relative">
              ...
              <div className="p-8 pt-10 flex flex-col items-center text-center relative z-10 border-b border-slate-200/80">
                {activeTicket.contact?.profilePictureUrl ? (
                  <img src={activeTicket.contact.profilePictureUrl} referrerPolicy="no-referrer" className="w-24 h-24 rounded-full object-cover shadow-md border-4 border-white mb-4" alt="Perfil" />
                ) : (
