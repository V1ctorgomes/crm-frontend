...
export default function ConfiguracoesPage() {
  ...
  const openQrCode = async (name: string) => {
    ...
    try {
      const res = await fetch(`${baseUrl}/instances/${name}/qrcode`);
      const data = await res.json();
      if (data && data.base64) {
        setActiveQrCode(data.base64);
      } else {
        alert("A instância já está conectada ou a carregar. Sincronize o status.");
        setIsQrModalOpen(false);
      }
    } catch (err) {
      alert("Falha ao gerar QR Code. Tente sincronizar o status primeiro.");
      setIsQrModalOpen(false);
    }
  };
              ...
              {activeTab === 'tema' && (
                <div className="animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {/* Dark e System botões omitidos por brevidade, pode recopiá-los da resposta anterior se quiser */}
                  </div>
                </div>
              )}
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                              {inst.name}
                              {inst.proxyHost && <span className="bg-purple-100 text-purple-700 text-[10px] px-2 py-0.5 rounded-full uppercase">Proxy</span>}
                            </h3>
