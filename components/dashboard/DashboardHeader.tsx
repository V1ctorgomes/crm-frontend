      ...
      <div className="flex items-center gap-3">
         <div className="bg-white border border-slate-200 px-3 py-2 rounded-md shadow-sm flex items-center gap-2.5">
           <div className="relative flex h-2 w-2">
              {isInstanceConnected && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-400 opacity-75"></span>}
              <span className={`relative inline-flex rounded-full h-2 w-2 ${isInstanceConnected ? 'bg-brand-600' : 'bg-red-500'}`}></span>
           </div>
           <span className="text-xs font-medium text-slate-600">{isInstanceConnected ? 'API Conectada' : 'Desconectado'}</span>
         </div>
         <Link href="/solicitacoes" className="bg-brand-600 text-white px-4 py-2 rounded-md font-medium shadow hover:bg-brand-700 transition-colors text-sm flex items-center gap-2">
