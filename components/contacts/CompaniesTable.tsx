...
export function CompaniesTable({ isLoading, companies, onOpen, onEdit, onDelete, pagination }: CompaniesTableProps) {
  return (
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/50 hover:bg-slate-50/50">
                <th className="h-12 px-4 align-middle font-medium text-slate-500">Empresa</th>
                ...
                companies.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="p-4 align-middle">
                      ...
                      >
                        <div className="w-9 h-9 rounded-md bg-brand-50 border border-brand-100 flex items-center justify-center text-brand-700 shrink-0">
                          <Building2 className="w-4 h-4" />
                        </div>
                        <span className="font-semibold text-brand-950 truncate max-w-[260px]">{c.legalName}</span>
                      </button>
                    </td>
                    <td className="p-4 align-middle text-slate-600 truncate max-w-[200px]">{c.tradeName || '--'}</td>
                    <td className="p-4 align-middle text-slate-600 font-mono text-[13px]">{formatCnpjInput(c.cnpj)}</td>
                    ...
                    <td className="p-4 align-middle text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => onOpen(c)}
                          className="h-8 px-2 rounded-md text-xs font-semibold text-brand-700 hover:bg-brand-50 transition-colors"
                          title="Ver perfil"
                        >
                          Abrir
                        </button>
                        <button
                          onClick={() => onEdit(c)}
                          className="h-8 w-8 rounded-md flex items-center justify-center text-slate-400 hover:text-brand-950 hover:bg-slate-100 transition-colors"
                          title="Editar"
                        >
