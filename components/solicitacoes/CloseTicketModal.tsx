    ...
    <div className="fixed inset-0 bg-brand-950/45 backdrop-blur-sm z-[9999] flex items-center justify-center p-4 animate-in fade-in duration-200" onMouseDown={onClose}>
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-slate-200" onMouseDown={e => e.stopPropagation()}>
        ...
        <div className="p-6 flex flex-col gap-5">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setCloseResolution('SUCCESS')} className={`flex flex-col items-center justify-center gap-2 p-3 rounded-lg border-2 transition-all ${closeResolution === 'SUCCESS' ? 'border-brand-600 bg-brand-50 text-brand-800' : 'border-slate-200 text-slate-500'}`}>
          ...
          <div className="space-y-2">
            ...
            <textarea className="flex min-h-[80px] w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-600/20 focus:border-brand-600 resize-none" value={closeReason} onChange={e => setCloseReason(e.target.value)} />
          </div>
        </div>
        <div className="flex items-center justify-end gap-2 p-6 pt-0 bg-slate-50 border-t border-slate-100 mt-2">
          ...
          <button onClick={() => onConfirm(closeResolution, closeReason)} className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors bg-brand-600 text-white hover:bg-brand-700 h-10 px-4 py-2">Confirmar</button>
        </div>
      </div>
    </div>
  );
