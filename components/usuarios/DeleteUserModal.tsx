...
export function DeleteUserModal({ user, onClose, onConfirm }: DeleteUserModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col space-y-2 p-6 pb-4 text-center overflow-y-auto">
          ...
          <p className="text-sm text-slate-500">
            Tem a certeza que pretende eliminar <b>{user.name}</b> da equipe? Esta ação revogará os seus acessos
            imediatamente.
          </p>
          <DeleteReasonFields value={reason} onChange={setReason} id="delete-user-reason" />
        </div>
        <div className="flex items-center justify-end gap-2 p-6 pt-0 bg-slate-50 border-t border-slate-100 mt-2 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-slate-100 hover:text-brand-950 border border-slate-200 h-10 px-4 py-2 bg-white"
          >
            Cancelar
          </button>
          <button
            ...
            className="flex-1 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors bg-red-600 text-white hover:bg-red-700 h-10 px-4 py-2 shadow-sm disabled:opacity-50 disabled:pointer-events-none"
          >
            Eliminar
          </button>
        </div>
