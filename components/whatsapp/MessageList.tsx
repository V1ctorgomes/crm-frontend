  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-2 z-10 no-scrollbar bg-slate-50/50">
      ...
      {filteredMessages.map((msg) => (
        <div key={msg.id} className={`max-w-[85%] md:max-w-[70%] w-fit relative px-3 py-2 rounded-xl flex flex-col break-words shadow-sm ${msg.fromMe ? 'self-end bg-brand-600 text-white rounded-tr-sm' : 'self-start bg-white border border-slate-200 text-slate-800 rounded-tl-sm'}`}>
          ...
          {msg.isMedia && msg.mediaData && (
            msg.mimeType?.startsWith('audio/') ? (
              ...
              <div className={`flex items-center gap-3 p-2 rounded-lg mb-1.5 mt-0.5 border ${msg.fromMe ? 'bg-brand-500 border-brand-400 text-white' : 'bg-slate-50 border-slate-200 text-slate-800'}`}>
                  <div className={`w-10 h-10 rounded-md flex items-center justify-center shrink-0 ${msg.fromMe ? 'bg-brand-400/50' : 'bg-white border border-slate-200'}`}>
                  ...
                  <div className="flex flex-col overflow-hidden w-full min-w-[120px] max-w-[200px]">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <span className={`text-[10px] font-mono uppercase ${msg.fromMe ? 'text-white/85' : 'text-slate-500'}`}>{msg.mimeType?.split('/')[1] || 'DOC'}</span>
                      <button onClick={() => setViewerMessage(msg)} className={`text-[10px] font-medium px-2 py-0.5 rounded cursor-pointer ${msg.fromMe ? 'bg-brand-500 hover:bg-brand-400' : 'bg-white border border-slate-200 hover:bg-slate-50'}`}>Abrir</button>
                    </div>
                  </div>
              </div>
            )
