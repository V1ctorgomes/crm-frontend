  ...
  <div className="fixed inset-0 bg-brand-950/45 backdrop-blur-sm z-[999] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
    <div className="bg-white rounded-xl shadow-lg w-full max-w-sm overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-slate-200" onClick={e => e.stopPropagation()}>
      ...
      <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-2">
        <button type="button" onClick={onClose} className="px-4 h-10 rounded-md font-medium text-slate-600 hover:bg-slate-100 border border-slate-200 bg-white transition-colors text-sm">Cancelar</button>
        <button type="button" onClick={() => void onConfirm()} className="bg-red-600 text-white px-4 h-10 rounded-md font-medium text-sm hover:bg-red-700 transition-colors">Excluir</button>
      </div>
    </div>
  </div>
);
...
export function DeleteMessageModal({ message, onClose, onConfirm }: DeleteMessageModalProps) {
  const preview = message.isMedia
    ? `Mídia${message.fileName ? `: ${message.fileName}` : ''}`
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl shadow-lg w-full max-w-sm overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 border border-slate-200"
      ...
      >
        <div className="p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-brand-950 mb-2">Apagar mensagem?</h3>
          <p className="text-sm text-slate-500">
            Esta mensagem será removida <strong>para todos</strong> no WhatsApp. Só é possível até <strong>50 horas</strong> após o envio.
          </p>
          <p className="mt-4 text-left text-xs font-medium text-slate-500 uppercase tracking-wide">Pré-visualização</p>
          <p className="mt-1 text-left text-sm text-slate-700 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 line-clamp-4 break-words">
            {preview}
          </p>
        </div>
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-2">
          ...
          <button
            ...
            className="bg-red-600 text-white px-4 h-10 rounded-md font-medium text-sm hover:bg-red-700 transition-colors"
          >
            Apagar para todos
          </button>
        </div>
